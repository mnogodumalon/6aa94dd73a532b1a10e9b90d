...
import type { Rechnungen, Auftraege, Kunden, LookupValue } from '@/types/app';
...
export type RechnungenDialogDefaults = Omit<Rechnungen['fields'], 'status'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  status: LOOKUP_OPTIONS['rechnungen']?.['status'] ?? [],
};
...
export function RechnungenDialog({ open, onClose, onSubmit, defaultValues, recordId, auftraegeList, kundenList, enablePhotoScan = true, enablePhotoLocation = true }: RechnungenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['rechnungsnummer', 'auftrag', 'kunde', 'rechnungsdatum', 'status'] as const;
    ...
    try {
      ...
      if (file && (file.type.startsWith('image/') || file.type === 'application/pdf')) {
        try {
          ...
          setFields(prev => ({ ...prev, pdf: fileUrl }));
        } catch (uploadErr) {
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'auftrag': (
      <div key="auftrag" className="space-y-1.5">
        ...
        <Combobox
          id="auftrag"
          placeholder="Welcher Auftrag wird berechnet?"
        ...
        {showErrors && !fields.auftrag && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('rechnungen', 'auftrag')}</p>
        )}
      </div>
    ),
    'kunde': (
      <div key="kunde" className="space-y-1.5">
        ...
        <Combobox
          id="kunde"
          placeholder="Welchem Kunden rechnen wir?"
    ...
    'faelligkeit': (
      <div key="faelligkeit" className="space-y-1.5">
        <Label htmlFor="faelligkeit">{fieldLabel('rechnungen', 'faelligkeit')}</Label>
        <DatePicker
          id="faelligkeit"
          placeholder="Bis wann ist die Zahlung fällig?"
          mode="date"
    ...
    'mwst': (
      <div key="mwst" className="space-y-1.5">
        <Label htmlFor="mwst">{fieldLabel('rechnungen', 'mwst')}</Label>
        <Input
          id="mwst"
          type="number"
          inputMode="decimal"
    ...
    'bruttobetrag': (
      <div key="bruttobetrag" className="space-y-1.5">
        <Label htmlFor="bruttobetrag">{fieldLabel('rechnungen', 'bruttobetrag')}</Label>
        <Input
          id="bruttobetrag"
          type="number"
          inputMode="decimal"
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        <Label htmlFor="status">{fieldLabel('rechnungen', 'status')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('rechnungen', 'status', 'offen') ?? 'Offen'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status) === 'bezahlt'}
            onClick={() => setFields(f => ({ ...f, status: (lookupKey(f.status) === 'bezahlt' ? undefined : 'bezahlt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status) === 'bezahlt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('rechnungen', 'status', 'bezahlt') ?? 'Bezahlt'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('rechnungen', 'status', 'storniert') ?? 'Storniert'}
          </button>
        </div>
        {showErrors && !fields.status && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('rechnungen', 'status')}</p>
        )}
      </div>
    ),
    'zahlungseingang': (
      <div key="zahlungseingang" className="space-y-1.5">
        <Label htmlFor="zahlungseingang">{fieldLabel('rechnungen', 'zahlungseingang')}</Label>
        <DatePicker
          id="zahlungseingang"
          placeholder="Wann ist die Zahlung eingegangen?"
          mode="date"
    ...
    'pdf': (
      <div key="pdf" className="space-y-1.5">
        <Label htmlFor="pdf">{fieldLabel('rechnungen', 'pdf')}</Label>
        {fields.pdf ? (
          <div className="flex items-center gap-3 rounded-lg border p-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm truncate text-foreground">{fields.pdf.split("/").pop()}</p>
              <div className="flex gap-2 mt-1">
                ...
                <button
                  ...
                  onClick={() => setFields(f => ({ ...f, pdf: undefined }))}
                >
                  {t('fr_remove')}
                </button>
              </div>
  ...
  const FIELD_LABELS: Record<string, string> = {"rechnungsnummer": "Rechnungsnummer", "auftrag": "Auftrag", "kunde": "Kunde", "rechnungsdatum": "Rechnungsdatum", "faelligkeit": "Fälligkeit", "nettobetrag": "Nettobetrag (€)", "mwst": "MwSt. (€)", "bruttobetrag": "Bruttobetrag (€)", "status": "Status", "zahlungseingang": "Zahlungseingang", "pdf": "Rechnungs-PDF"};
  const CURRENCY_KEYS = new Set<string>(["nettobetrag", "mwst", "bruttobetrag"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"auftrag": {"auftragsnummer": "Auftragsnummer", "fahrzeug": "Fahrzeug", "annahmedatum": "Annahmedatum", "fertigstellungstermin": "Zugesagter Fertigstellungstermin", "kundenwunsch": "Kundenwunsch", "status": "Status", "mechaniker": "Zuständiger Mechaniker", "kilometerstand_annahme": "Kilometerstand bei Annahme (km)", "notizen": "Notizen"}, "kunde": {"vorname": "Vorname", "nachname": "Nachname", "kundentyp": "Kundentyp", "telefon": "Telefon", "email": "E-Mail", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "notizen": "Notizen"}};
