/**
 * FahrzeugeDialog — pre-generated create/edit dialog for Fahrzeuge.
 *
 ...
 * FahrzeugeDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Fahrzeuge['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<FahrzeugeDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Fahrzeuge';
...
/** Widened prefill type for FahrzeugeDialog.defaultValues — see file header. */
export type FahrzeugeDialogDefaults = Fahrzeuge['fields'];

interface FahrzeugeDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Fahrzeuge['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: FahrzeugeDialogDefaults;
...
export function FahrzeugeDialog({ open, onClose, onSubmit, defaultValues, recordId, kundenList, enablePhotoScan = true, enablePhotoLocation = true }: FahrzeugeDialogProps) {
  const [fields, setFields] = useState<Partial<Fahrzeuge['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['kennzeichen', 'marke', 'modell', 'halter'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Fahrzeuge['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'fahrzeuge');
      await onSubmit(clean as Fahrzeuge['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "kennzeichen": string | null, // Kennzeichen\n  "marke": string | null, // Marke\n  "modell": string | null, // Modell\n  "erstzulassung": string | null, // YYYY-MM-DD\n  "kilometerstand": number | null, // Kilometerstand (km)\n  "fahrgestellnummer": string | null, // Fahrgestellnummer\n  "hu_faellig": string | null, // YYYY-MM-DD\n  "halter": string | null, // Display name from Kunden (see <available-records>)\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["halter"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const halterName = raw['halter'] as string | null;
        if (halterName) {
          const halterMatch = kundenList.find(r => matchName(halterName!, [[r.fields.vorname ?? '', r.fields.nachname ?? ''].filter(Boolean).join(' ')]));
          if (halterMatch) merged['halter'] = createRecordUrl(APP_IDS.KUNDEN, halterMatch.record_id);
        }
        return merged as Partial<Fahrzeuge['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('fahrzeuge') })
    : t('new_entity', { entity: appLabel('fahrzeuge') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'kennzeichen': (
      <div key="kennzeichen" className="space-y-1.5">
        <Label htmlFor="kennzeichen">{fieldLabel('fahrzeuge', 'kennzeichen')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="kennzeichen"
          placeholder="z. B. B-FR-2025"
          value={fields.kennzeichen ?? ''}
          onChange={e => setFields(f => ({ ...f, kennzeichen: e.target.value }))}
          required
        />
        {showErrors && !fields.kennzeichen && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('fahrzeuge', 'kennzeichen')}</p>
        )}
      </div>
    ),
    'marke': (
      <div key="marke" className="space-y-1.5">
        ...
        <Input
          ...
          required
        />
        {showErrors && !fields.marke && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('fahrzeuge', 'marke')}</p>
        )}
      </div>
    ),
    'modell': (
      <div key="modell" className="space-y-1.5">
        ...
        <Input
          id="modell"
          placeholder="z. B. Golf 8 Comfortline"
          value={fields.modell ?? ''}
          onChange={e => setFields(f => ({ ...f, modell: e.target.value }))}
          required
        />
        {showErrors && !fields.modell && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('fahrzeuge', 'modell')}</p>
        )}
      </div>
    ),
    'erstzulassung': (
      <div key="erstzulassung" className="space-y-1.5">
        <Label htmlFor="erstzulassung">{fieldLabel('fahrzeuge', 'erstzulassung')}</Label>
        <DatePicker
          id="erstzulassung"
          placeholder="Wann wurde das Auto zugelassen?"
          mode="date"
    ...
    'kilometerstand': (
      <div key="kilometerstand" className="space-y-1.5">
        <Label htmlFor="kilometerstand">{fieldLabel('fahrzeuge', 'kilometerstand')}</Label>
        <Input
          id="kilometerstand"
          type="number"
          inputMode="decimal"
    ...
    'hu_faellig': (
      <div key="hu_faellig" className="space-y-1.5">
        <Label htmlFor="hu_faellig">{fieldLabel('fahrzeuge', 'hu_faellig')}</Label>
        <DatePicker
          id="hu_faellig"
          placeholder="Wann ist die HU fällig?"
          mode="date"
    ...
    'halter': (
      <div key="halter" className="space-y-1.5">
        <Label htmlFor="halter">{fieldLabel('fahrzeuge', 'halter')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="halter"
          placeholder="Welcher Kunde ist der Halter?"
          items={kundenListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.vorname ?? r.record_id),
          }))}
          value={extractRecordId(fields.halter)}
          onChange={id => setFields(f => ({ ...f, halter: id ? createRecordUrl(APP_IDS.KUNDEN, id) : undefined }))}
          onCreateNew={(q) => openCreateKunden("halter", q)}
          createLabel={t('create_in', { entity: appLabel('kunden') })}
        />
        {showErrors && !fields.halter && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('fahrzeuge', 'halter')}</p>
        )}
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"kennzeichen": "Kennzeichen", "marke": "Marke", "modell": "Modell", "erstzulassung": "Erstzulassung", "kilometerstand": "Kilometerstand (km)", "fahrgestellnummer": "Fahrgestellnummer", "hu_faellig": "HU fällig", "halter": "Halter (Kunde)"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"halter": {"vorname": "Vorname", "nachname": "Nachname", "kundentyp": "Kundentyp", "telefon": "Telefon", "email": "E-Mail", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "notizen": "Notizen"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.FAHRZEUGE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
