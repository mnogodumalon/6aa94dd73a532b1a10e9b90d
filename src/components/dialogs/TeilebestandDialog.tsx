/**
 * TeilebestandDialog — pre-generated create/edit dialog for Teilebestand.
 *
 ...
 * TeilebestandDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Teilebestand['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<TeilebestandDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Teilebestand } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Teilebestand';
...
/** Widened prefill type for TeilebestandDialog.defaultValues — see file header. */
export type TeilebestandDialogDefaults = Teilebestand['fields'];

interface TeilebestandDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Teilebestand['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: TeilebestandDialogDefaults;
...
export function TeilebestandDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: TeilebestandDialogProps) {
  const [fields, setFields] = useState<Partial<Teilebestand['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['artikelnummer', 'bezeichnung'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Teilebestand['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'teilebestand');
      await onSubmit(clean as Teilebestand['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "artikelnummer": string | null, // Artikelnummer\n  "bezeichnung": string | null, // Bezeichnung\n  "hersteller": string | null, // Hersteller\n  "einkaufspreis": number | null, // Einkaufspreis (€)\n  "verkaufspreis": number | null, // Verkaufspreis (€)\n  "bestand": number | null, // Bestand (Stück)\n  "mindestbestand": number | null, // Mindestbestand (Stück)\n  "lagerplatz": string | null, // Lagerplatz\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Teilebestand['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('teilebestand') })
    : t('new_entity', { entity: appLabel('teilebestand') });
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'artikelnummer': (
      <div key="artikelnummer" className="space-y-1.5">
        ...
        <Input
          id="artikelnummer"
          placeholder="z. B. LZ-001234"
          value={fields.artikelnummer ?? ''}
          onChange={e => setFields(f => ({ ...f, artikelnummer: e.target.value }))}
          required
        />
        {showErrors && !fields.artikelnummer && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('teilebestand', 'artikelnummer')}</p>
        )}
      </div>
    ),
    'bezeichnung': (
      <div key="bezeichnung" className="space-y-1.5">
        ...
        <Input
          id="bezeichnung"
          placeholder="z. B. Luftfilter 3er Pack"
          value={fields.bezeichnung ?? ''}
          onChange={e => setFields(f => ({ ...f, bezeichnung: e.target.value }))}
          required
        />
        {showErrors && !fields.bezeichnung && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('teilebestand', 'bezeichnung')}</p>
        )}
      </div>
    ),
    'hersteller': (
    ...
    'einkaufspreis': (
      <div key="einkaufspreis" className="space-y-1.5">
        <Label htmlFor="einkaufspreis">{fieldLabel('teilebestand', 'einkaufspreis')}</Label>
        <Input
          id="einkaufspreis"
          type="number"
          inputMode="decimal"
    ...
    'verkaufspreis': (
      <div key="verkaufspreis" className="space-y-1.5">
        <Label htmlFor="verkaufspreis">{fieldLabel('teilebestand', 'verkaufspreis')}</Label>
        <Input
          id="verkaufspreis"
          type="number"
          inputMode="decimal"
    ...
    'bestand': (
      <div key="bestand" className="space-y-1.5">
        <Label htmlFor="bestand">{fieldLabel('teilebestand', 'bestand')}</Label>
        <Input
          id="bestand"
          type="number"
          inputMode="decimal"
    ...
    'mindestbestand': (
      <div key="mindestbestand" className="space-y-1.5">
        <Label htmlFor="mindestbestand">{fieldLabel('teilebestand', 'mindestbestand')}</Label>
        <Input
          id="mindestbestand"
          type="number"
          inputMode="decimal"
  ...
  const FIELD_LABELS: Record<string, string> = {"artikelnummer": "Artikelnummer", "bezeichnung": "Bezeichnung", "hersteller": "Hersteller", "einkaufspreis": "Einkaufspreis (€)", "verkaufspreis": "Verkaufspreis (€)", "bestand": "Bestand (Stück)", "mindestbestand": "Mindestbestand (Stück)", "lagerplatz": "Lagerplatz"};
  const CURRENCY_KEYS = new Set<string>(["einkaufspreis", "verkaufspreis"]);
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.TEILEBESTAND} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
