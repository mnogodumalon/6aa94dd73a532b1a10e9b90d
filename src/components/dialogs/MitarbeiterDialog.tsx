/**
 * MitarbeiterDialog — pre-generated create/edit dialog for Mitarbeiter.
 *
 ...
 * MitarbeiterDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Mitarbeiter['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<MitarbeiterDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Mitarbeiter, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Mitarbeiter';
...
/** Widened prefill type for MitarbeiterDialog.defaultValues — see file header. */
export type MitarbeiterDialogDefaults = Omit<Mitarbeiter['fields'], 'rolle' | 'status'> & {
    rolle?: LookupValue | string;
...
interface MitarbeiterDialogProps {
  ...
  defaultValues?: MitarbeiterDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  rolle: LOOKUP_OPTIONS['mitarbeiter']?.['rolle'] ?? [],
  status: LOOKUP_OPTIONS['mitarbeiter']?.['status'] ?? [],
};
...
export function MitarbeiterDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MitarbeiterDialogProps) {
  ...
  const REQUIRED_FIELDS = ['vorname', 'nachname', 'rolle', 'status'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Mitarbeiter['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'mitarbeiter');
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "email": string | null, // E-Mail\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "rolle": LookupValue | null, // Rolle (select one key: "annahme" | "mechaniker" | "chef") mapping: annahme=Annahme, mechaniker=Mechaniker, chef=Chef\n  "stundensatz": number | null, // Stundensatz (€)\n  "status": LookupValue | null, // Status (select one key: "aktiv" | "krank" | "urlaub") mapping: aktiv=Aktiv, krank=Krank, urlaub=Urlaub\n}`;
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('mitarbeiter') })
    : t('new_entity', { entity: appLabel('mitarbeiter') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'email': (
      <div key="email" className="space-y-1.5">
        <Label htmlFor="email">{fieldLabel('mitarbeiter', 'email')}</Label>
        <Input
          id="email"
          type="email"
          inputMode="email"
          placeholder="z. B. frank@werkstatt.de"
          value={fields.email ?? ''}
    ...
    'vorname': (
      <div key="vorname" className="space-y-1.5">
        ...
        <Input
          id="vorname"
          placeholder="z. B. Frank"
          value={fields.vorname ?? ''}
          onChange={e => setFields(f => ({ ...f, vorname: e.target.value }))}
          required
        />
        {showErrors && !fields.vorname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('mitarbeiter', 'vorname')}</p>
        )}
      </div>
    ),
    'nachname': (
      <div key="nachname" className="space-y-1.5">
        ...
        {showErrors && !fields.nachname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('mitarbeiter', 'nachname')}</p>
        )}
      </div>
    ),
    'rolle': (
      <div key="rolle" className="space-y-1.5">
        <Label htmlFor="rolle">{fieldLabel('mitarbeiter', 'rolle')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.rolle) === 'annahme'}
            onClick={() => setFields(f => ({ ...f, rolle: (lookupKey(f.rolle) === 'annahme' ? undefined : 'annahme') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.rolle) === 'annahme'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('mitarbeiter', 'rolle', 'annahme') ?? 'Annahme'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.rolle) === 'mechaniker'}
            onClick={() => setFields(f => ({ ...f, rolle: (lookupKey(f.rolle) === 'mechaniker' ? undefined : 'mechaniker') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.rolle) === 'mechaniker'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('mitarbeiter', 'rolle', 'mechaniker') ?? 'Mechaniker'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.rolle) === 'chef'}
            onClick={() => setFields(f => ({ ...f, rolle: (lookupKey(f.rolle) === 'chef' ? undefined : 'chef') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.rolle) === 'chef'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('mitarbeiter', 'rolle', 'chef') ?? 'Chef'}
          </button>
        </div>
        {showErrors && !fields.rolle && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('mitarbeiter', 'rolle')}</p>
        )}
      </div>
    ),
    'stundensatz': (
      <div key="stundensatz" className="space-y-1.5">
        <Label htmlFor="stundensatz">{fieldLabel('mitarbeiter', 'stundensatz')}</Label>
        <Input
          id="stundensatz"
          type="number"
          inputMode="decimal"
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        <Label htmlFor="status">{fieldLabel('mitarbeiter', 'status')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('mitarbeiter', 'status', 'aktiv') ?? 'Aktiv'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('mitarbeiter', 'status', 'krank') ?? 'Krank'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('mitarbeiter', 'status', 'urlaub') ?? 'Urlaub'}
          </button>
        </div>
        {showErrors && !fields.status && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('mitarbeiter', 'status')}</p>
        )}
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"email": "E-Mail", "vorname": "Vorname", "nachname": "Nachname", "rolle": "Rolle", "stundensatz": "Stundensatz (€)", "status": "Status"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.MITARBEITER} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
