 ...
 * teilebestandList (full hook array — resolves the Teilebestand applookup),
...
import type { Auftragspositionen, Auftraege, Mitarbeiter, Teilebestand, LookupValue } from '@/types/app';
...
import { TeilebestandDialog } from '@/components/dialogs/TeilebestandDialog';
...
export type AuftragspositionenDialogDefaults = Omit<Auftragspositionen['fields'], 'positionstyp'> & {
    positionstyp?: LookupValue | string;
  };
...
export function AuftragspositionenDialog({ open, onClose, onSubmit, defaultValues, recordId, auftraegeList, mitarbeiterList, teilebestandList, enablePhotoScan = true, enablePhotoLocation = true }: AuftragspositionenDialogProps) {
  ...
  // Inline-Create state for "Teilebestand" target. The dropdown's
  // "+ Neuer …" option opens a sub-dialog; on submit we POST, add the new
  // record to the local `extraTeilebestand` list, and select it in
  // the originating Combobox via the captured `createTeilebestandField`.
  const [createTeilebestandOpen, setCreateTeilebestandOpen] = useState(false);
  const [createTeilebestandInitial, setCreateTeilebestandInitial] = useState('');
  const [createTeilebestandField, setCreateTeilebestandField] = useState<string>('');
  const [extraTeilebestand, setExtraTeilebestand] = useState< Teilebestand[]>([]);
  const teilebestandListAll = useMemo(
    () => [...teilebestandList, ...extraTeilebestand],
    [teilebestandList, extraTeilebestand],
  );
  function openCreateTeilebestand(fieldKey: string, q: string) {
    setCreateTeilebestandField(fieldKey);
    setCreateTeilebestandInitial(q);
    setCreateTeilebestandOpen(true);
  }
  const [showErrors, setShowErrors] = useState(false);
  const REQUIRED_FIELDS = ['auftrag', 'positionstyp', 'bezeichnung', 'menge', 'einzelpreis'] as const;
  ...
  const computedContext = useMemo<ComputedContext>(() => ({
    lookupLists: {
      ...
      'teil': teilebestandList,
    },
  }), [auftraegeList, mitarbeiterList, teilebestandList, ]);
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="teil" entity="Teilebestand">\n${JSON.stringify(teilebestandList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "auftrag": string | null, // Display name from Aufträge (see <available-records>)\n  "positionstyp": LookupValue | null, // Positionstyp (select one key: "arbeit" | "teil") mapping: arbeit=Arbeit, teil=Teil\n  "bezeichnung": string | null, // Bezeichnung\n  "menge": number | null, // Menge\n  "einzelpreis": number | null, // Einzelpreis (€)\n  "mechaniker": string | null, // Display name from Mitarbeiter (see <available-records>)\n  "dauer_stunden": number | null, // Dauer (Stunden)\n  "teil": string | null, // Display name from Teilebestand (see <available-records>)\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["auftrag", "mechaniker", "teil"]);
        ...
        const teilName = raw['teil'] as string | null;
        if (teilName) {
          const teilMatch = teilebestandList.find(r => matchName(teilName!, [String(r.fields.artikelnummer ?? '')]));
          if (teilMatch) merged['teil'] = createRecordUrl(APP_IDS.TEILEBESTAND, teilMatch.record_id);
        }
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'positionstyp': (
      <div key="positionstyp" className="space-y-1.5">
        <Label htmlFor="positionstyp">{fieldLabel('auftragspositionen', 'positionstyp')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.positionstyp) === 'arbeit'}
            onClick={() => setFields(f => ({ ...f, positionstyp: (lookupKey(f.positionstyp) === 'arbeit' ? undefined : 'arbeit') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.positionstyp) === 'arbeit'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('auftragspositionen', 'positionstyp', 'arbeit') ?? 'Arbeit'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.positionstyp) === 'teil'}
            onClick={() => setFields(f => ({ ...f, positionstyp: (lookupKey(f.positionstyp) === 'teil' ? undefined : 'teil') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.positionstyp) === 'teil'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('auftragspositionen', 'positionstyp', 'teil') ?? 'Teil'}
          </button>
        </div>
        {showErrors && !fields.positionstyp && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('auftragspositionen', 'positionstyp')}</p>
        )}
      </div>
    ),
    'bezeichnung': (
      <div key="bezeichnung" className="space-y-1.5">
        ...
        <Input
          id="bezeichnung"
          placeholder="z. B. Bremsschleiferei, Wechsel Filteranlage"
          value={fields.bezeichnung ?? ''}
          onChange={e => setFields(f => ({ ...f, bezeichnung: e.target.value }))}
          required
        />
        {showErrors && !fields.bezeichnung && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('auftragspositionen', 'bezeichnung')}</p>
        )}
      </div>
    ),
    'menge': (
    ...
    'einzelpreis': (
      <div key="einzelpreis" className="space-y-1.5">
        <Label htmlFor="einzelpreis">{fieldLabel('auftragspositionen', 'einzelpreis')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="einzelpreis"
          type="number"
          inputMode="decimal"
        ...
        />
        {showErrors && !fields.einzelpreis && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('auftragspositionen', 'einzelpreis')}</p>
        )}
      </div>
    ),
    'mechaniker': (
      <div key="mechaniker" className="space-y-1.5">
        ...
        <Combobox
          id="mechaniker"
          placeholder="Welcher Mechaniker führt die Arbeit durch?"
    ...
    'dauer_stunden': (
      <div key="dauer_stunden" className="space-y-1.5">
        <Label htmlFor="dauer_stunden">{fieldLabel('auftragspositionen', 'dauer_stunden')}</Label>
        <Input
          id="dauer_stunden"
          type="number"
          inputMode="decimal"
    ...
    'teil': (
      <div key="teil" className="space-y-1.5">
        <Label htmlFor="teil">{fieldLabel('auftragspositionen', 'teil')}</Label>
        <Combobox
          id="teil"
          placeholder="Welches Teil wird verwendet?"
          items={teilebestandListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.artikelnummer ?? r.record_id),
          }))}
          value={extractRecordId(fields.teil)}
          onChange={id => setFields(f => ({ ...f, teil: id ? createRecordUrl(APP_IDS.TEILEBESTAND, id) : undefined }))}
          onCreateNew={(q) => openCreateTeilebestand("teil", q)}
          createLabel={t('create_in', { entity: appLabel('teilebestand') })}
        />
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"auftrag": "Auftrag", "positionstyp": "Positionstyp", "bezeichnung": "Bezeichnung", "menge": "Menge", "einzelpreis": "Einzelpreis (€)", "mechaniker": "Mechaniker", "dauer_stunden": "Dauer (Stunden)", "teil": "Verwendetes Teil"};
  const CURRENCY_KEYS = new Set<string>(["einzelpreis"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"auftrag": {"auftragsnummer": "Auftragsnummer", "fahrzeug": "Fahrzeug", "annahmedatum": "Annahmedatum", "fertigstellungstermin": "Zugesagter Fertigstellungstermin", "kundenwunsch": "Kundenwunsch", "status": "Status", "mechaniker": "Zuständiger Mechaniker", "kilometerstand_annahme": "Kilometerstand bei Annahme (km)", "notizen": "Notizen"}, "mechaniker": {"email": "E-Mail", "vorname": "Vorname", "nachname": "Nachname", "rolle": "Rolle", "stundensatz": "Stundensatz (€)", "status": "Status"}, "teil": {"artikelnummer": "Artikelnummer", "bezeichnung": "Bezeichnung", "hersteller": "Hersteller", "einkaufspreis": "Einkaufspreis (€)", "verkaufspreis": "Verkaufspreis (€)", "bestand": "Bestand (Stück)", "mindestbestand": "Mindestbestand (Stück)", "lagerplatz": "Lagerplatz"}};
  ...
  return (
    ...
    {createTeilebestandOpen && (
      <TeilebestandDialog
        open={createTeilebestandOpen}
        onClose={() => setCreateTeilebestandOpen(false)}
        onSubmit={async (newFields) => {
          const result = await LivingAppsService.createTeilebestandEntry(newFields as any) as { id?: string };
          if (result?.id) {
            const newRec = { record_id: result.id, fields: newFields } as unknown as Teilebestand;
            setExtraTeilebestand(prev => [...prev, newRec]);
            const url = createRecordUrl(APP_IDS.TEILEBESTAND, result.id);
            setFields(prev => ({ ...prev, [createTeilebestandField]: url } as any));
          }
          setCreateTeilebestandOpen(false);
        }}
        defaultValues={createTeilebestandInitial
          ? ({ artikelnummer: createTeilebestandInitial } as any)
          : undefined}
      />
    )}
    </>
