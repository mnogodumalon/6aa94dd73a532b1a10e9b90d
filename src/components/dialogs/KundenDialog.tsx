...
export type KundenDialogDefaults = Omit<Kunden['fields'], 'kundentyp'> & {
...
export function KundenDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: KundenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['vorname', 'nachname', 'kundentyp'] as const;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'vorname': (
      <div key="vorname" className="space-y-1.5">
        ...
        {showErrors && !fields.vorname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kunden', 'vorname')}</p>
        )}
      </div>
    ),
    'nachname': (
      <div key="nachname" className="space-y-1.5">
        ...
        {showErrors && !fields.nachname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('kunden', 'nachname')}</p>
        )}
      </div>
    ),
    'kundentyp': (
      <div key="kundentyp" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kundentyp) === 'privat'}
            onClick={() => setFields(f => ({ ...f, kundentyp: (lookupKey(f.kundentyp) === 'privat' ? undefined : 'privat') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kundentyp) === 'privat'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('kunden', 'kundentyp', 'privat') ?? 'Privat'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kundentyp) === 'gewerbe'}
            onClick={() => setFields(f => ({ ...f, kundentyp: (lookupKey(f.kundentyp) === 'gewerbe' ? undefined : 'gewerbe') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kundentyp) === 'gewerbe'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('kunden', 'kundentyp', 'gewerbe') ?? 'Gewerbe'}
          </button>
        </div>
    ...
    'telefon': (
      <div key="telefon" className="space-y-1.5">
        ...
        <Input
          ...
          type="tel"
          inputMode="tel"
          placeholder="z. B. 089 123 456789"
    ...
    'email': (
      <div key="email" className="space-y-1.5">
        <Label htmlFor="email">{fieldLabel('kunden', 'email')}</Label>
        <Input
          id="email"
          type="email"
          inputMode="email"
    ...
    'notizen': (
      <div key="notizen" className="space-y-1.5">
        ...
        <Textarea
          id="notizen"
          placeholder="Besonderheiten, Rabattvereinbarung, Zahlungsbedingungen..."
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "kundentyp": "Kundentyp", "telefon": "Telefon", "email": "E-Mail", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "notizen": "Notizen"};
